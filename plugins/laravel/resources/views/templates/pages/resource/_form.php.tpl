            ...
            <div class="controls">
                ...
                <% } else if (_.indexOf(formElements, field.type)) { %>
                    <% if (field.type == 'select') { %>
                        <?php if (
                            isset($data_options['<%= field.tableName %>'])
                            && $data_options['<%= field.tableName %>']->count() > 0
                        ) { ?>
                            <select name="<%= field.name %>"
                                ...
                                <?php foreach ($data_options['<%= field.tableName %>'] as $option) { ?>
                                    <option value="<?php echo $option->id; ?>"><?php echo $option->id; ?></option>
                                <?php } ?>
                            </select>
                            <a class="btn btn-success" href="<?php echo route('<%= field.routeName %>.create'); ?>">
                                <?php echo _("Add option"); ?>
                            </a>
                        <?php } else { ?>
                            <?php echo _("No options,"); ?> <a href="<?php echo route('<%= field.routeName %>.create'); ?>"><?php echo _("why not add some?"); ?></a>
                        <?php } ?>
                    <% } %>
                <% } else { %>
