...
// Overview
const PLUGIN_NAME = 'slush-regenerator:generate-config';
...
function plugin(options)
{
    var stream = through2.obj( function(file, enc, cb) {
        ...
        var subStreams = {
            /**
              * Create formatting config stream
              */
            createFormattingConfig: {
                read: fs.createReadStream( blueprint.templatePath('formatting.php.tpl') ),
                write: function(templateFileContents)
                {
                    ...
                    magic.detect(templateFileContents, function(err, mimeType) {
                        ...
                        var newFile = new File({ // blueprint.file
                            ...
                            path: blueprint.filename('formatting', fileExtension)
                        });
                        stream.push(newFile);
